...
function buildHowToDoc() {
  const children = [
    ...
    numbered('In Intake, open id.land Connection and sign in. Do this before anything else — without it, owner names come back as "Blurred".'),
    numbered('Still in Intake, choose a county and enter a property address or parcel/APN.'),
    ...
    numbered('Open the Deals tab, click the new deal, review entitlement score and route.'),
    numbered('Open the Silos tab and drag the card into the lane that fits the play.'),
    numbered('Go to Outreach → Build Mail List → print labels or export CSV (no DNC vendor needed).'),
    numbered('Press Sync Now in the header so the rest of the team sees your pipeline.'),
    ...
    body('Header buttons: Theme (light/dark), A− and A+ (text size), Sign Out, Copy App Link (share with the team), How to Use (the in-app version of this guide), Feature Request, Intel (jump to the moat panel), and Sync Now (force an immediate team sync).'),
    ...
    h3('id.land connection (do this first)'),
    bullet('Enter your id.land email and password, or paste a Bearer token under Advanced.'),
    bullet('Token stays in your browser only — never stored in the repo, and never synced in the team bundle.'),
    bullet('A live token upgrades parcel geometry, land use, and market comps from MOCK to LIVE.'),
    bullet('Sign-in is per browser: signing in on your laptop does not sign you in on your phone or iPad.'),
    bullet('The status line shows when the session expires. If it reads "Session expired", sign in again — sessions do not last indefinitely.'),
    bullet('Tell-tale sign you are not connected: owner names read "Blurred" and comps will not load.'),

    h1('2. Pipeline — Manage Deals'),
    h3('Views'),
    bullet('Table view: columns for address, county, acres, $/acre, status, and route.'),
    bullet('Board view: Trello-style cards you can drag between stages — also the easier view on a phone.'),
    bullet('Click any row or card to open the full deal modal.'),
    bullet('Search covers address, portfolio, owner, tags, and notes. Filter by county, route, or portfolio.'),
    ...
    body('Every deal gets a route. On the Silos tab, drag a card between lanes to assign it — the deal updates as soon as you drop it. You can also set the route from the deal modal.'),
    bullet('Development & Entitlement: sewer/utilities and builder spillover support entitlement plays'),
    bullet('Direct Sale: comp-supported acquisition where sewer or entitlement is the bottleneck'),
    bullet('Retail Brokerage: smaller tracts or straight brokerage/listing plays'),
    body('Each lane header shows its deal count and total acres. Entitlement-bottlenecked tracts are excluded from outreach queues automatically.'),
    ...
    bullet('How to Use: the header button opens a short in-app version of this guide, so nobody has to find this file.'),
    ...
    body('The Intake county dropdown currently covers 16 counties: Williamson, Travis, Bastrop, Caldwell, Hays, Comal, Bexar, Guadalupe, Gillespie, Blanco, Burnet, Bandera, Kendall, Llano, Fort Bend, and Smith (Tyler).'),
    ...
    table([
      ...
      ['Owner name shows "Blurred"', 'id.land not connected, or that session expired — re-sign in on the Intake panel'],
      ['id.land says "Session expired"', 'Normal — access tokens lapse. Enter email and password again'],
      ['Comps will not load', 'Same cause: check the id.land status line is green'],
