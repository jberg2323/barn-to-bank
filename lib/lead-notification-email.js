...
function fromAddress() {
  // Trimmed because these values are pasted into the Vercel dashboard and pick up
  // stray whitespace; a newline in a From header is rejected outright by Resend.
  return (process.env.LEAD_FROM_EMAIL
    || process.env.FEATURE_REQUEST_FROM_EMAIL
    || 'Barn to Bank <intake@cto.com>').trim();
}
...
function buildEmailContent(lead) {
  ...
  const appUrl = process.env.LEAD_APP_URL || 'https://barntobanklandgroup.com/app';
  ...
  const html = `
    <div style="font-family: -apple-system, system-ui, sans-serif; max-width: 640px; line-height: 1.5; color: #1c241d;">
      ...
      <p style="margin: 0 0 20px; color: #6d7568;">Submitted from the public contact form at barntobanklandgroup.com.</p>
