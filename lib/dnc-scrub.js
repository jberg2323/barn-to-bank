...
async function liveScrubPhones(phones) {
  ...
  if (!key) {
    const err = new Error(
      'DNC scrub not configured. Set DNC_SCRUB_API_KEY in Vercel (loginId from dncscrub.com → User Admin → Get API Key).',
    );
  ...
  try {
    ...
    const err = new Error(`DNCScrub API error (${res.status}): ${text.slice(0, 240)}`);
  ...
  if (!Array.isArray(rows)) {
    const err = new Error('DNCScrub API returned an unexpected response format');
    err.code = 'UPSTREAM_ERROR';
    throw err;
  }
...
async function scrubPhones(phones) {
  ...
  return { results, live: true, provider: PROVIDER };
}
